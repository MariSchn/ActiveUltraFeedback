...
PREFERENCE_ANNOTATION_SYSTEM_PROMPT = """You are an expert judge LLM. Your role is to impartially and critically evaluate text quality based on a given criteria. You'll receive an input with two sections, enclosed in tags: <INSTRUCTION>...</INSTRUCTION> for the instruction, and <TEXT>...</TEXT> for the text output. Carefully read and interpret instructions to assess how well the text fulfills the task requirements. Provide annotation for the text with a rating. Output only a single number (the rating) and nothing else."""
...
**Output Format**: Write out only the rating number (1-5) without any additional text or explanation.
...
**Output Format**: Write out only the rating number (1-5) or N/A without any additional text or explanation.
