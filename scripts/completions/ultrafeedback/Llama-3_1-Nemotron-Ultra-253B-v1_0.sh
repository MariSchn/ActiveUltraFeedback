...
#SBATCH --job-name=Llama-3_1-Nemotron-Ultra-253B-v1_0
#SBATCH --output=./logs/completions/Llama-3_1-Nemotron-Ultra-253B-v1/%j.out
...
export VLLM_SERVER_CONCURRENCY_LIMIT=50
num_nodes_per_instance=4
...
python -u -m activeuf.generate_completions \
  --dataset_path /iopsstor/scratch/cscs/smarian/datasets/allenai/ultrafeedback_binarized_cleaned/train_prefs \
  --model_name nvidia/Llama-3_1-Nemotron-Ultra-253B-v1 \
  --model_class vllm_server \
  --output_path /iopsstor/scratch/cscs/smarian/datasets/completions/Llama-3_1-Nemotron-Ultra-253B-v1_0 \
  --seed 7122 \
