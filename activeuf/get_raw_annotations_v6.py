...
from activeuf.prompts_v6 import *

# these are not system prompts, these are user prompts.
ASPECT2ANNOTATION_PROMPT = {
    "instruction_following": INSTRUCTION_FOLLOWING_ANNOTATION_USER_PROMPT,
    "honesty": HONESTY_ANNOTATION_USER_PROMPT,
    "truthfulness": TRUTHFULNESS_ANNOTATION_USER_PROMPT,
    "helpfulness": HELPFULNESS_ANNOTATION_USER_PROMPT,
}
...
"Qwen/Qwen3-32B"
Example run command:
    python -m activeuf.get_raw_annotations_v6 \
...
if __name__ == "__main__":
    ...
    for prompt_completion in tqdm(dataset):
        # find the response of meta-llama/Llama-3.3-70B-Instruct
        for completion in prompt_completion["completions"]:
            if completion["model"] == args.model_name:
                example_completion = completion["response_text"]
                break
            ...
            for completion in prompt_completion["completions"]:
                if completion["model"] != args.model_name:
                    ...
                    messages = [
                        ...
                        {
                            "role": "user",
                            "content": annotation_prompt.format(prompt=prompt_completion["prompt"], completion=completion["response_text"],
                                                                example=example_completion),
                        }
                    ]
    ...
    with open("show_messages.txt", 'w') as f:
        for messages in all_messages:
            for sub_message in messages:
                f.write(str(sub_message) + "\n\n")
            f.write(
                "\n\n****************************************************************************\n\n")

    with open("show_messages.json", 'w') as f:
        for messages in all_messages:
            for sub_message in messages:
                f.write(str(sub_message) + "\n\n")
            f.write(
    ...
    print(len(all_messages[0]))
    # TODO: make saving possbile so you can look at the results later.
    # exit(0)
    ...
    for meta, score in zip(all_metadata, all_raw_annotations):
        key = (meta["prompt_id"], meta["model_name"])
        aspect_scores[key][meta["aspect"]] = score  # score should be int

    def update_overall_scores(example):
        prompt_id = example["prompt_id"]
        completions = example["completions"]
        for comp in completions:
            key = (prompt_id, comp["model"])
            # Insert the aspect dictionary as overall_score
            comp["overall_score"] = aspect_scores.get(key, {})
        return example
