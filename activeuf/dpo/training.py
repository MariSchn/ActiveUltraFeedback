...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--train_path", type=str,
                        help="The path to the local dataset to use for training (e.g. trl-lib/ultrafeedback_binarized/train)")
    parser.add_argument("--test_path", type=str,
                        help="The path to the local dataset to use for testing (e.g. data/trl-lib/ultrafeedback_binarized/test)")
    parser.add_argument("--dataset_path", type=str,
                        help="The HuggingFace path of the dataset to use for training and testing (e.g. trl-lib/ultrafeedback_binarized)")
    parser.add_argument("--config", type=str,
                        help="The path to the YAML training config.")

    parser.add_argument("--model_path", type=str,
                        help="The HuggingFace path of the model to train (e.g. Qwen/Qwen2-0.5B-Instruct)")
    ...
    parser.add_argument("--output_dir", type=str, default="models/dpo/",
                        help="Where to export the tokenizer, trained model, and training args")
...
if __name__ == "__main__":
    ...
    dataset = load_from_disk(args.dataset_path) if os.path.exists(
        args.dataset_path) else load_dataset(args.dataset_path)["train_prefs"]
    ...
    trainer_config = DPOConfig(
        ...
        per_device_train_batch_size=math.ceil(training_config.get(
            "train_batch_size", 8) / accelerator.num_processes),
        per_device_eval_batch_size=math.ceil(training_config.get(
            "eval_batch_size", 8) / accelerator.num_processes),
