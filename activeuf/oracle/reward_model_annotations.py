...
if __name__ == "__main__":
    ...
    parser.add_argument("--annotation_model", type=str, required=False, default="Skywork/Skywork-Reward-V2-Llama-3.1-8B")
    ...
    def filter_completions(example):
        example["completions"] = [c for c in example["completions"] if c["model"] == args.model_to_annotate]
        return example
    ...
    def annotate(example):
        ...
        conv = [{"role": "user", "content": example["prompt"]}, {"role": "assistant", "content": model_completion["response_text"]}]
    ...
    with open(f"{args.output_path}/{args.model_to_annotate.split('/')[-1]}/first_sample.json", "w") as f:
