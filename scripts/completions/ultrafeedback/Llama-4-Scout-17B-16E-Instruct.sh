...
#SBATCH --job-name=Llama-4-Scout-17B-16E-Instruct
#SBATCH --output=./logs/completions/Llama-4-Scout-17B-16E-Instruct/%j.out
#SBATCH --environment=activeuf_new_xformers
...
export VLLM_SERVER_CONCURRENCY_LIMIT=300
...
python -u -m activeuf.generate_completions \
  ...
  --output_path /iopsstor/scratch/cscs/smarian/datasets/completions/Llama-4-Scout-17B-16E-Instruct \
  --seed 6768 \
