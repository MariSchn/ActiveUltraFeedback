...
#SBATCH --output=./logs/annotation/qwen_3_235b/%j.out
...
python -u -m activeuf.oracle.get_raw_annotations \
    ...
    --max_tokens 24000 \
    --output_path /iopsstor/scratch/cscs/smarian/datasets/4_annotated_completions/skywork_with_small/qwen_3_235b \
...
EOF
  ...
  echo "Submitted job for model: $MODEL using Qwen 3 235B for skywork"
...
python -u -m activeuf.oracle.get_raw_annotations \
    ...
    --model_name="Qwen/Qwen3-235B-A22B" \
    --max_tokens 24000 \
    --output_path /iopsstor/scratch/cscs/smarian/datasets/4_annotated_completions/ultrafeedback_with_small/qwen_3_235b \
    --model_class vllm_server \
    --temperature 0.0 \
    --top_p 0.1 \
    --num_nodes \$num_nodes_per_instance \
...
EOF
  ...
  echo "Submitted job for model: $MODEL using Qwen 3 235B for ultrafeedback"
