...
class ENNModelConfig:
    ...
    head_num_layers: int = field(metadata={
        "help": "The number of layers in each MLP of the ensemble head."
    })
    head_hidden_dim: int = field(metadata={
...
class ENNTrainerConfig:
    ...
    lr_scheduler_type: str = field(metadata={
        "help": "Type of learning rate scheduler.",
        "choices": ["linear", "cosine"],
    })
    learning_rate: float = field(metadata={"help": "Initial learning rate."})
    max_steps: int = field(metadata={
        "help": "Maximum number of training steps per outer batch added."})
    ...
    center_rewards_coefficient: float | None = field(metadata={
    ...
    report_to: str = field(
        metadata={"help": "Reporting tool for the trainer."})
...
class ENNRegularizationConfig:
    initial_value: float = field(metadata={
        "help": "Strength of regularization towards initial weights."
    })
    decay_type: str = field(metadata={
...
@dataclass
class ENNConfig:
    previous_checkpoint_path: str | None = field(metadata={
    ...
    model: ENNModelConfig = field(
        metadata={"help": "Configuration for the ENN model."})
...
class LoopConfig:
    ...
    oracle_name: str = field(metadata={
        "help": "Oracle scorer for response texts.", 
        "choices": ["random", "ultrafeedback"],
    })
    acquisition_function_type: str = field(metadata={
        "help": "Acquisition function type", 
        "choices": ["random", "dts", "infomax", "maxminlcb", "infogain"],
    })
    reward_model_type: str = field(metadata={
        "help": "Reward model to train.", "choices": ["none", "enn"],
    })
    ...
    replay_buffer_factor: int = field(
        metadata={"help": "Replay buffer for reward model training will contain up to (replay_buffer_factor * outer_loop_batch_size) samples."})
    ...
    enn: ENNConfig | None = field(metadata={
        "help": "All configs related to ENN reward model and training."})
...
def get_loop_args() -> argparse.Namespace:
    ...
    if args.reward_model_type != "none":
        ...
        if not hasattr(trainer_args, "output_dir"):
            setattr(trainer_args, "output_dir", f"{config_dict['base_trainer_dir']}/{args.run_id}")
